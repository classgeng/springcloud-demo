...
public class PaginationQueryTask extends RecursiveTask<List<UserInfo>> {
    ...
    public PaginationQueryTask(String tenantId, String systemSource, int startPage, int endPage, DemoService demoService) {
    ...
    protected List<UserInfo> compute() {
        if (endPage - startPage < 4) {
            ...
            for (int pageNo = startPage; pageNo <= endPage; pageNo++) {
                ...
                try {
                    invoicePageForExport = demoService.queryUserInfoList(tenantId,systemSource);
            ...
            PaginationQueryTask left = new PaginationQueryTask(tenantId, systemSource, startPage, midPage, demoService);
            PaginationQueryTask right = new PaginationQueryTask(tenantId, systemSource, midPage + 1, endPage, demoService);
